    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-fade-in">
      ...
      <div className={`relative w-full max-w-5xl max-h-[95vh] overflow-hidden ${THEME.cardBg} rounded-3xl shadow-2xl flex flex-col md:flex-row animate-slide-down`}>
        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-start overflow-y-auto md:max-h-[95vh] hide-scrollbar relative pb-36 scroll-smooth overscroll-contain">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#2D241E] dark:text-white mb-3">{item.name}</h2>
          ...
          <p className={`text-sm sm:text-base leading-relaxed mb-6 ${THEME.muted}`}>{item.description}</p>
  ...
  return (
    <div className={`group flex flex-row sm:flex-col justify-between gap-3 sm:gap-0 ${THEME.cardBg} rounded-2xl p-4 border ${THEME.border} shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 animate-fade-in relative w-full overflow-hidden cursor-pointer`} onClick={() => onQuickView(item, 'menu', { variantName: selectedVariant?.name, prepOption: item.prepOptions?.[0] || '' })}>
      <div className="relative w-2/5 sm:w-full shrink-0 aspect-[4/5] sm:aspect-square rounded-xl overflow-hidden bg-black/5 dark:bg-white/5">
        ...
        {item.isPopular && (
          <div className="absolute bottom-2 left-2 bg-gradient-to-r from-orange-500 to-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-md z-10 flex items-center gap-1 max-w-[calc(100%-0.75rem)] truncate">
      ...
      <div className="w-3/5 sm:w-full min-w-0 flex flex-col justify-between gap-3">
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className={`text-[10px] sm:text-xs font-semibold ${THEME.muted} truncate`}>{item.type}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-3 gap-1.5 w-full">
            {item.variants.map(v => (
              <button
                ...
                className={`flex flex-col items-center justify-center px-1 py-2.5 min-h-[46px] rounded-xl border transition-all w-full overflow-hidden ${selectedVariant?.name === v.name ? 'border-[#6F4E37] bg-[#6F4E37]/10 dark:bg-[#D4B895]/10 dark:border-[#D4B895]' : 'border-black/10 dark:border-white/10 hover:border-black/30 dark:hover:border-white/30'}`}
              >
                <span className={`font-bold text-[11px] sm:text-xs md:text-sm leading-tight truncate w-full text-center ${selectedVariant?.name === v.name ? 'text-[#6F4E37] dark:text-[#D4B895]' : 'text-[#2D241E] dark:text-[#EAE6DF]'}`}>{v.name}</span>
                {v.desc && <span className={`text-[9px] sm:text-[10px] md:text-[11px] mt-0.5 truncate w-full text-center ${selectedVariant?.name === v.name ? 'text-[#6F4E37] dark:text-[#D4B895] opacity-90' : 'text-[#8A7B72] dark:text-[#A89F95]'}`}>{v.desc}</span>}
              </button>
            ))}
          </div>
          )}
          ...
          <div className="flex items-center justify-between gap-2 pt-3 border-t border-black/5 dark:border-white/10">
            <span className="text-lg sm:text-xl md:text-2xl font-bold text-[#2D241E] dark:text-[#EAE6DF] shrink-0">{formatPrice(currentPrice)}</span>

            {cartQuantity > 0 ? (
              <div className="flex items-center justify-between bg-[#6F4E37] text-white rounded-xl px-1.5 py-1 shadow-md animate-fade-in w-full max-w-[138px] min-h-[46px]">
                <button onClick={handleDecrement} className="p-1.5 hover:bg-white/20 rounded-lg transition-colors min-h-[34px] min-w-[34px] flex items-center justify-center"><Minus size={15} strokeWidth={2.5}/></button>
                <span className="text-sm font-bold w-5 text-center">{cartQuantity}</span>
...
const FavoritesPage = ({ favorites, cart, isFavOpen, onClose, onViewCart, addToCart, updateQuantity, toggleFavorite, onQuickView }) => {
  ...
  const [instructions, setInstructions] = useState('');
  const [customs, setCustoms] = useState({});
  const ratingInfo = viewingItem ? getItemRating(viewingItem.id) : null;
  const currentPrice = viewingItem && selectedVariant ? selectedVariant.price : (viewingItem ? viewingItem.price : 0);
  const isFav = viewingItem ? favorites.includes(viewingItem.id) : false;
  ...
  const handleAddToCart = () => {
    if (viewingItem) {
      addToCart(viewingItem, selectedVariant, viewingItem.description, customs, instructions, viewingItem.prepOptions?.[0] || '');
      handleCloseItem();
    }
  };
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-[#FAF7F2] dark:bg-[#12100E] p-4 sm:p-6 animate-fade-in">
      <div className={`relative w-full max-w-5xl h-full max-h-[95vh] overflow-hidden ${THEME.cardBg} rounded-3xl shadow-2xl p-6 md:p-10 flex flex-col`}>
        ...
        <div className="mt-4 pt-4 border-t border-black/5 dark:border-white/5 flex justify-center">
          <button onClick={onClose} className="px-6 py-3 rounded-full bg-[#6F4E37] text-white font-bold hover:bg-[#5A3E2B] transition-colors">
            Add more Items
          </button>
        </div>
        ...
        {viewingItem ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
            {favoriteItems.map(item => (
              <div key={`fav-${item.id}`} className={`relative flex flex-col p-4 rounded-2xl border ${THEME.border} ${THEME.cardBg} shadow-sm group hover:shadow-lg transition-all w-full cursor-pointer`} onClick={() => handleViewItem(item)}>
                <button onClick={(e) => { e.stopPropagation(); toggleFavorite(item.id); }} className={`absolute top-3 right-3 p-2 bg-white/80 dark:bg-black/50 backdrop-blur-md rounded-full shadow-sm hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500 transition-colors z-10 min-h-[40px] min-w-[40px] flex items-center justify-center`} title="Remove Favorite">
                   <Trash2 size={16} />
                </button>
                <div className="flex gap-4 mb-4">
                  ...
                  <div className="flex flex-col justify-center flex-1 pr-8 min-w-0">
                    <div>
                      <h4 className="font-bold text-lg sm:text-xl leading-tight text-[#2D241E] dark:text-white line-clamp-2">{item.name}</h4>
                      <p className={`text-xs sm:text-sm font-semibold ${THEME.primaryText} mt-1`}>{formatPrice(item.price)}</p>
                    </div>
                  </div>
                </div>
                {(() => {
